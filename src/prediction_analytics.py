...
class PredictionAnalytics:  # TODO(Jack) give a better name and move its behavior to correct places
    ...
    def extract_and_transform(self, X, Y, network_runner):
        """Extract and transform the data regarding an rbf networks predictions, to a relational model"""
        # Load the network data
        n = X.shape[0]
        correct, incorrect = network_runner.all_correct_incorrect(X, Y)
        new_ordering = np.concatenate([correct.indices, incorrect.indices])
        ...
        dimension_stat = np.array([point_ids_nd, dim_ids, selected_z_diff, selected_tau, selected_z, selected_z_bar],
                                  dtype=object)
        return point_stat, dimension_stat

    def roc_curve(self, X, Y, network_runner):
        ...
        final_tprs = self.combine(class_wise_tprs)
        final_fprs = self.combine(class_wise_fprs)

        return final_tprs, final_fprs

    def combine(self, prs):
        k = len(prs)
        min_ss = self._find_min_length(prs)
    ...
    def _find_min_length(self, arrays):
        return min([arr.shape[0] for arr in arrays])

    def not_roc_curve(self, X, Y, network_runner):
        ...
        ranked_prob_correct = -np.sort(-correct.a)
        ...
        for i in xrange(0, n):
            num_recalled = i + 1
            thresh = correct.a[i]
            falsely_recalled = np.sum(np.greater_equal(incorrect.a, thresh).astype(np.int32))
